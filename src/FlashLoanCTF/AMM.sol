...
contract AMM {
    ...
    constructor(address _lendToken) payable {
        ...
        SafeERC20.safeTransferFrom(lendToken, msg.sender, address(this), 400e18);
    }

    function swapLendTokenForEth(address to) external returns (uint256 ethAmountOut) {
    ...
    function swapEthForLendToken(address to) external payable returns (uint256 lendTokenAmountOut) {
        // TAKE advantage of "donations" and avoid locked tokens
    ...
    function getLendTokenToEthPrice(uint256 _lendTokenAmountIn) public view returns (uint256 _ethAmountOut) {
        _ethAmountOut = (ethReserve * _lendTokenAmountIn) / (lendTokenReserve + _lendTokenAmountIn);
    }

    function getEthToLendTokenPrice(uint256 _ethAmountIn) public view returns (uint256 lendTokenAmountOut) {
        lendTokenAmountOut = (lendTokenReserve * _ethAmountIn) / (ethReserve + _ethAmountIn);
    }
